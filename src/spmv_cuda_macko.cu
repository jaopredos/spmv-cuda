        ...
        double           *y)
{
    ...
    for (int c = first; c < last; c++) {
        for (int k = 0; k < MACKO_CHUNK_SIZE; k++) {
            int col = chunks[c].cols[k];
            if (col != -1)
                acc += chunks[c].vals[k] * x[col];
        }
    }
    y[row] = acc;
}
        ...
        double           *y)
{
    ...
    for (int c = lane; c < nchunks; c += 32) {
        ...
        for (int k = 0; k < MACKO_CHUNK_SIZE; k++) {
            int col = chunks[chunk_idx].cols[k];
            if (col != -1)
                acc += chunks[chunk_idx].vals[k] * x[col];
        }
    }
...
CUDAMACKOCtx *cuda_macko_ctx_create(const MACKOMatrix *A, const double *x) {
    ...
    CUDA_CHECK(cudaMalloc(&ctx->d_chunks,     A->total_chunks * sizeof(MACKOChunk)));
    ...
    CUDA_CHECK(cudaMemcpy(ctx->d_chunks,     A->chunks,     A->total_chunks * sizeof(MACKOChunk), cudaMemcpyHostToDevice));
...
float cuda_macko_run(CUDAMACKOCtx *ctx) {
    ...
    kernel_macko_warp_per_row<<<ctx->blocks, ctx->tpb>>>(
        ctx->rows, ctx->d_chunks, ctx->d_row_ptr, ctx->d_row_nchunks,
        ctx->d_x, ctx->d_y);
